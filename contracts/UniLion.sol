...
contract UniLion is Context, IBEP20, Ownable {
    ...
    string private _name = "UniLion";
    string private _symbol = "LION";
    uint8 private _decimals = 9;


    uint256 public _taxFee = 5; // Reflect
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 4; // Liq + Marketing
    ...
    constructor (address pancakeRouterAddr, address[] memory sniperAddresses) {
        _rOwned[_msgSender()] = _rTotal;
        pancakeRouterAddress = pancakeRouterAddr;
        IPancakeRouter02 _pancakeRouter = IPancakeRouter02(pancakeRouterAddr);
        pancakePair = IPancakeFactory(_pancakeRouter.factory())
            .createPair(address(this), _pancakeRouter.WETH());
        pancakeRouter = _pancakeRouter;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        blacklistSniperAddresses(sniperAddresses);
    ...
    function swapTokens(uint256 amount) private lockTheSwap {
        uint256 liquidityFee = _liquidityFee.sub(marketingDivisor);                                                       
        uint256 marketingTokens = amount.div(_liquidityFee).mul(marketingDivisor);
        uint256 liquidityTokens = amount.div(_liquidityFee).mul(liquidityFee);
        swapTokensMarketing(marketingTokens);
        swapTokensAddLiquidity(liquidityTokens);
    }

    function swapTokensMarketing(uint256 amount) private lockTheSwap {
        uint256 initialBalance = address(this).balance;
        swapTokensForBNB(amount);
        uint256 newBalance = address(this).balance.sub(initialBalance);
        transferToAddressBNB(marketingAddress, newBalance);
    }

    function swapTokensAddLiquidity(uint256 amount) private lockTheSwap {
    ...
    function withdrawBNB(address account, uint256 amount) public onlyOwner {
        transferToAddressBNB(payable(account), amount);
    }
